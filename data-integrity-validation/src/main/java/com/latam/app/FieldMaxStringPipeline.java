...
public class FieldMaxStringPipeline {

	private static final Logger LOG = LoggerFactory.getLogger(FieldMaxStringPipeline.class);
	...
	static class CompareResults extends DoFn<Integer, Boolean> {
		...
		public void processElement(@Element Integer result, OutputReceiver<Boolean> out) {
			...
			if(expected.get().toString().equals(result.toString())) {
				LOG.info("The Max String is a valid. Result: " + result.toString() + " / Expected: " + expected.get().toString());
				out.output(true);
			}else {
				LOG.info("The Max String " + result.toString() + " does not match the expected value of " + expected.get().toString());
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		customOptions.setJobName("fieldmaxsringpipeline" + new Date().getTime());
